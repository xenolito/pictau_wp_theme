...
function pictau_copyright($atts){
	...
	// $output .= 'Copyright © '. apply_shortcodes('[myYear]') . ' '. strtoupper(get_bloginfo( 'name' )) .' | '. __('Powered by','pictau') .'&nbsp;<a class="pictau-logo" href="https://www.pictau.com" target="_blank"> PICTAU &nbsp;<span class="pcticon-pictau-logo"><span class="path1"></span><span class="path2"></span><span class="path3"></span></span></a>';
	$output .= '	<div class="copy">';
	$output .= '		Copyright © '. apply_shortcodes('[myYear]') . ' '. strtoupper(get_bloginfo( 'name' ));
	$output .= '	</div>';
...
function video_as_background( $atts = [], $content = '' ) {
    ...
    extract(shortcode_atts(array(
		...
		$output .= '<video class="video-bg" '. $isAutoplay .' loop playsinline muted preload="metadata" poster="'. wp_upload_dir()['baseurl'] .'/'. $src .'.webp" aria-label="intro video">';
		$output .= '<source src="'. wp_upload_dir()['baseurl'] .'/'. $src .'.mp4" type="video/mp4">';
		if ( $webm ) $output .= '<source src="'. wp_upload_dir()['baseurl'] .'/'. $src .'.webm" type="video/webm">';
		$output .= '</video></div>';
...
//! AREAS NOT FEATURED ONLY --> FOR MEGAMENU
function show_areas_not_featured( $atts = [], $content = '' ) {

	extract(shortcode_atts(array(
		"limit" 	=> 0,
	), $atts));

	$output = ' <div class="areas-not-featured">';
	// $output .= '	<h2>SOLUCIONES PARA TU NEGOCIO</h2>';
	...
	$output .= '	<div class="areas-not-featured-grid">';
	...
	if ( $pods->total() > 0 ) {
		while ($pods->fetch()) {
			...
			if (!$featured) {
				$output .= '<a href="'. $link .'">';
				$output .= '	<div class="area-not-featured-item">';
				$output .= '		<div class="area-not-featured-item-icon">';
				$output .= 				wp_svg_inline_filter($icon_output);
				$output .= '		</div>';
				$output .= '		<div class="area-not-featured-item-content">';
				$output .= '			<h3>' . $nombre . '</h3>';
				$output .= '			<p>' . $description . '</p>';
				$output .= '		</div>';
				$output .= '	</div>';
				$output .= '</a>';
				// $output .= 'nombre: ' . $nombre . '<br> | link: ' . $link . '<br> | logo: ' . $icon . '<br> | description: ' . $description . '<br><hr><br>';
			}
...
add_shortcode( 'areas-not-featured', 'show_areas_not_featured' );

//! GET AREAS SOLUTIONS
function show_area_solutions( $atts = [], $content = '' ) {

	extract(shortcode_atts(array(
		"limit" 	=> 0,
	), $atts));

	$output = '<div class="areas-featured">';
	$output .= '<h2>AREA INCLUDED SOLUTIONS</h2><br>';
...
add_shortcode( 'area_solutions', 'show_area_solutions' );
...
//! ALL AREAS LIST FOR FOOTER MENU (with icon svg inline)
function all_areas_list( $atts = [], $content = '' ) {
...
add_shortcode( 'areas-list', 'all_areas_list' );


//! FEATURERD AREAS CARDS FOR HOMEPAGE WITH POD TEMPLATE

function show_areas_featured_cards( $atts = [], $content = '' ) {
...
add_shortcode( 'areas-featured-cards', 'show_areas_featured_cards' );

//! OVERLAPPING ALL AREAS CARDS FOR HOMEPAGE WITH POD TEMPLATE

function show_areas_cards( $atts = [], $content = '' ) {
	...
	$output .= do_shortcode('[pods name="area" template="Area Featured Card for Home page"]');

	return $output;
}
add_shortcode( 'areas-cards', 'show_areas_cards' );




//! OVERLAPPING ONLY FEATURERD AREAS CARDS FOR HOMEPAGE
function home_featured_areas_cards( $atts = [], $content = '' ) {
...
// add_shortcode( 'featured-cards', 'home_featured_areas_cards' );
...
function casos_exito_grid( $atts = [], $content = '' ) {
	...
	if ( $pods->total() > 0 ) {
		...
		$output .= '<div class="cursor pulse">
									<div class="content">
										<span>' . __('Ver Vídeo', 'pictau') . '</span>
									</div>
								</div>';
		...
		while ($pods->fetch()) {
			...
			$testimonial_initial = $pods->field('testimonial_initial');
			...
			$output .= '<div class="caso-exito-item">';
			$output .= '	<a href="'. $link .'" data-mouse_pointer_id="'. __('Ver Vídeo', 'pictau') .'">';
			...
			$output .=  '			<quote>'. $testimonial_initial . '</quote>';
			$output .=  '			<div class="signature">';
			$output .=  				'<span>' . $responsable . '</span>';
			$output .=  				'<span>' . $cargo . '</span>';
			$output .=  '			</div>';
			$output .=  '		</div>';
			// $output .=  $video. '<br>';
			...
			$output .= '	<div class="wp-block-buttons is-layout-flex wp-block-buttons-is-layout-flex">
											<div class="wp-block-button">
												<a href="'. $link .'" class="wp-block-button__link wp-element-button">Ver vídeo</a>
											</div>
      							</div>';
			$output .= '</div>';
...
//! GET RELATED SOLUTIONS SECTION FOR CPT "solucion"
function related_solutions( $atts = [], $content = '' ) {
	...
	$output = do_shortcode('[pods name="solucion" id="'. get_the_id() .'" template="Soluciones Relacionadas"]');

	return wp_svg_inline_filter($output);
}
add_shortcode( 'soluciones-relacionadas', 'related_solutions' );
...
// $str = '<img decoding="async" width="145" height="97" src="//localhost:3000/xen_media/cliente-aon.webp" class="attachment-width size-width" alt="">';
// $result = extractWidthHeight($str);
...
/*
<figure class="wp-block-image size-large" data-filter="servicios">
	<img fetchpriority="high" decoding="async" width="300" height="200" src="//localhost:3000/xen_media/logo-Perez-Llorca.webp" alt="Pérez-Llorca" class="wp-image-2469">
</figure>
*/
...
// [pods name="area" template="Mobile menu all Solutions Areas list"]


// wp_svg_inline_filter()
...
function getFormattedFecha($date) {
	...
	// echo '<h1>String?: '. gettype($date) .'</h1><br>' . $date;

	// $fecha = wp_date($date);
	// $dia = wp_date('d', strtotime($fecha));
	// $mes = wp_date('F', strtotime($fecha));
	// $ano = wp_date('Y', strtotime($fecha));
	// $hora = wp_date('G', strtotime($fecha)) - get_option('gmt_offset');
	// $minutos = wp_date('i', strtotime($fecha));
...
//! EVENT CPT HTML DATE TAG
function event_date_tag( $atts = [], $content = '' ) {
	...
	if ($post_type !== 'evento') return '⛔️ NO SE PUEDE MOSTRAR FECHA, NO ES UN CPT EVENTO';

	// // $tipo_evento = $pod->display('tipo_evento') ===;

	$lugar = $pod->field('webinar')[0] === '0' ? $pod->field('ciudad') : 'webinar';

	// echo gettype($pod->field('fecha_inicio'));

	$fecha_inicio = getFormattedFecha($pod->field('fecha_inicio'));
	$fecha_fin = getFormattedFecha($pod->field('fecha_fin'));
	$variation = intval($hours) ? 'show-hours' : '';
	$show_location = intval($location) ? true : false;
	$mes_inicio = intval($shortmonth) ? substr($fecha_inicio['mes'], 0, 3) : $fecha_inicio['mes'];


	$output = ' 		<card class="fecha '. $variation .'">';
	$output .= ' 			<div class="short-date">';
	$output .= ' 				<div class="dia">';
	$output .=  					$fecha_inicio['dia'];
	$output .= ' 				</div>';
	$output .= ' 				<div class="mes">';
	$output .=  					$mes_inicio;
	$output .= ' 				</div>';
	$output .= ' 				<div class="year">';
	$output .=  					$fecha_inicio['ano'];
	$output .= ' 				</div>';
	$output .= ' 			</div>';
	$output .= ' 			<div class="extended-date">';
	...
	$output .=  						$fecha_inicio['hora'] . $fecha_inicio['minutos'] . ' ' . __('to','pictau') . ' ' . $fecha_fin['hora'] . $fecha_fin['minutos'] . ' h';
	// $output .=  						$fecha_inicio['hora'] . $fecha_inicio['minutos']. ' ' . __('to','pictau') . ' ' . $fecha_fin['hora'];
	$output .= ' 				</div>';
	...
	return $output;
	// return 'EVENT-DATE SHORTCODE';
}
add_shortcode( 'event-date', 'event_date_tag' );


//! EVENT CPT LANDING HERO TITLE
function event_hero_title( $atts = [], $content = '' ) {
	...
	$webinar = $pod->field('webinar')[0];
	$webinar_class = ($webinar) ? 'is-webinar' : '';

	$logo = ($pod->field('has_event_logo') === '1' || $webinar  )  ? '<figure class="'. $webinar_class .'"><img src="' . $pod->field('event_logo._src') . '" alt="'. $pod->field('nombre') .'" ></figure>' : '<h2 class="wp-block-heading">' . $pod->field('titulo') . '</h2>';

	$output = '';
	$output .= ' 		<div class="titulo">';
	$output .=  			$logo;
	if ($webinar) {
	$output .= ' 			<h2 class="wp-block-heading">'. $pod->field('nombre') .'</h2>';
	}
	$output .= ' 		</div>';
...
add_shortcode( 'event-hero-title', 'event_hero_title' );

//! CPT EVENT LANDING + FORM
function inscripcion_evento( $atts = [], $content = '' ) {
	...
	$logo = ($pod->field('has_event_logo') === '1')  ? '<figure><img src="' . $pod->field('event_logo._src') . '" alt="'. $pod->field('nombre') .'" ></figure>' : $pod->field('titulo');

	$has_logo_class = ($pod->field('has_event_logo') === '1') ? 'has-logo' : '';

	$webinar = $pod->field('webinar')[0];
	$webinar_class = ($webinar) ? 'is-webinar' : '';
	$lugar = $pod->field('ciudad');

	$fecha_inicio = getFormattedFecha($pod->field('fecha_inicio'));
	$fecha_fin = getFormattedFecha($pod->field('fecha_fin'));

	$output = '<div class="event-landing-apply">';
	$output .= '	<div class="content">';
	$output .= '		<header class="apply-header">';
	$output .= '			<div class="apply-header-description ' . $has_logo_class . ' '.  $webinar_class . '">';
	$output .= 					$logo;
	$output .= '			</div>';
	$output .= ' 		</header>';
	$output .= 			do_shortcode('[event-date shortmonth="1" hours="1" location="1"]');

	if (!$webinar) {
	$output .= ' 		<card class="address">';
	$output .=  		$pod->field('address');
	$output .= ' 		</card>';
	}
	$output .= ' 		<card class="precio">';
	$output .=  		$pod->field('precio');
	$output .= ' 		</card>';
	$output .= ' 		<div class="wp-block-buttons is-layout-flex wp-block-buttons-is-layout-flex">
        						<div class="wp-block-button">
											<a href="#modal-inscripcion" data-form_content="'. $pod->field('nombre') .'" class="wp-block-button__link wp-element-button">Inscribirme</a>
										</div>
      						</div>';
	$output .= '	</div>';
	$output .= '</div>';
...
add_shortcode( 'inscripcion-evento', 'inscripcion_evento' );



//! SLIDER FOR WEBINARS ON DEMAND
function webinars_slider( $atts = [], $content = '' ) {
	...
	// $output = '<div class="slider-wrapper" data-mouse_pointer>';
	$output = '<div class="slider-webinars">';
	...
	$pods = pods('webinar-ondemand', $params);

	if ( $pods->total() > 0 ) {

		while ($pods->fetch()) {
			// $featured = $pods->field('home_featured')[0];
			$nombre = $pods->field('post_title');
			$link = $pods->field('permalink');
			$thumb = '<img loading="eager" src="'. $pods->field('post_thumbnail_url.large') .'" alt="'. $nombre .'" >';
			...
			// $description = $pods->field('description');
				...
				$output .= '	<a href="'. $link .'">';
				$output .= '		<div>';
				// $output .= 				$pods->field('post_thumbnail.large');
				$output .= 				$thumb;
				$output .= '		</div>';
				$output .= '	</a>';
				$output .= '</div>';
...
add_shortcode( 'webinar-slider', 'webinars_slider' );
