...
document.addEventListener('DOMContentLoaded', () => {
	...
	const HeaderAnimation = class {
		constructor(targetDOMElement, config = {}) {
			...
			this.triggerstart = !triggerstart ? `${this.animation === 'slideFromBottom' && this.whattoanim === 'self' ? 'top-=110%' : 'top'} bottom-=15%` : `top-=110% ${triggerstart}`
		...
		setupAnimation = () => {
			...
			if (this.autoplay && !this.chainedTo) {
				...
				this.trigger = ScrollTrigger.create({
					...
					// invalidateOnRefresh: true, //! Test this, bette not to use when there is no scrub associated to ST
				})
			}
